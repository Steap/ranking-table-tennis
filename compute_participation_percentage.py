...
#            pid_new_players.append(pid)
    ...
    for pid in pid_participation_list:
        assoc = players[pid].association
        assoc = assoc.lower()
        if participation_assoc.get(assoc):
            participation_assoc[assoc] += 1
        else:
            participation_assoc[assoc] = 1
            
    participation_city = {}    
    for pid in pid_participation_list:
        city = players[pid].city
        city = city.lower()
        if participation_city.get(city):
            participation_city[city] += 1
        else:
            participation_city[city] = 1
    
    print(tournament.name)
    print(tournament.location)
    print(participation_assoc)
    print(participation_city)
    print('')
